...
(function(angular) {
    ...
    var teLoading = function($compile) {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                var tpl = angular.element('<span class="te-loading disable' + small + '" ng-hide="!loading"><i class="material-icons te-i noI te-i-spin">autorenew</i></span>');
                element.append(tpl);
    ...
    var ADMtrvDirective = function (ADMtrv, constants, $q, $http, $sce) {
        return {
            scope: {
                configs: '=',
                selected: '=?',
            ...
            require: 'ngModel',
            templateUrl: $sce.trustAsResourceUrl(CDNServer + 'ADMtrvDrv.html'),
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                if (scope.configs.init) 
                    scope.configs.init(scope);
                ...
                scope.canBeOpen = function (item, depth) {
                    return (scope.options.onDemand || !scope.options.readOnly || scope.options.readOnly && item[scope.$chn] && item[scope.$chn].length) && scope.depthValid(depth);
                }
                ...
                var onPapaOpen = function(node, item, object) {
                    scope.$applyAsync(function () {
                        node[scope.$chn] = object;
                        item.kidContentLoading = false;
                    });
                }
                
                scope.openTriggered = function(node, item, open) {
                    if (!open || !scope.options.onPapaOpen) return;
                    var result = scope.options.onPapaOpen(node);
                    if (isPromise(result)) {
                        item.kidContentLoading = true;
                        $q.when(result).then(function (res) {
                            onPapaOpen(node, item, res);
                        });
                    }
                    else if (result instanceof Array)
                        onPapaOpen(node, item, result);
                }
    ...
    return angular.module('ADM-treeView', [])
        ...
        .directive('admTrv', ['ADMtrv', 'constants', '$q', '$http', '$sce', ADMtrvDirective])

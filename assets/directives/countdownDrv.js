...
define(['app'], function (app) {
    ...
    app.register.directive('countdown', [function () {
        return {
            restrict: 'A',
            scope: {},
            replace: true,
            ...
            link: function (scope, element, attrs) {
                ...
                FlipClock.prototype.setDimensions = function () {
                    var flipHeight = this.options.containerElement.height(),
                        flipWidth = Math.floor(flipHeight / 1.5);
                ...
                scope.$on('$destroy', function () {
                    if (scope.ctrl.flip)
                        scope.ctrl.flip.stop();
                });

                attrs.$observe("countdown", function (_newVal) {
                    if (!_newVal) return;
                    scope.$applyAsync(function () {
                        if (scope.ctrl.flip)
                            scope.ctrl.flip.stop();

                        scope.ctrl.flip = new FlipClock({
                            isCountdown: true,
                            startTime: _newVal,
